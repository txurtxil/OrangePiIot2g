...
REMOTE_ADDRESS=$(whiptail --inputbox "IP publica remota" 8 78 Blue --title "Conexion ssh inverso" 3>&1 1>&2 2>&3)
REMOTE_PORT=$(whiptail --inputbox "Introduce el puerto de tu srv ssh " 8 78 Blue --title "Conexion ssh inverso" 3>&1 1>&2 2>&3)
PASSWORD=$(whiptail --inputbox "Introduce la PASSWORD de tu srv ssh " 8 78 Blue --title "Conexion ssh inverso" 3>&1 1>&2 2>&3)

       	FWD_PORT=1234
	WAIT_SECONDS=60
        # si el numero de argumentos no es igual a 3 ejecutamos esta ayuda y salimos:
#	if [ "$#" != "2" ]; then
